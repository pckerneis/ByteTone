/*
  ==============================================================================
    ...
    vm.c
    Created: 17 Dec 2022 10:58:05pm
    Author:  pcker
...
static BtlValue clockNative(int argCount, BtlValue* args)
{
...
static BtlValue peek(int distance)
{
  return vm.stackTop[-1 - distance];
}

static bool callValue(BtlValue callee, int argCount)
{
  if (IS_OBJ(callee)) {
...
#define BINARY_BITWISE_OP(valueType, op) \
  ...
  for (;;) {
    ...
    switch (instruction = READ_BYTE())
    {
      case OP_CONSTANT:
      {
        BtlValue constant = READ_CONSTANT();
        push(constant);
        break;
      }
